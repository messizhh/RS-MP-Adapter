...
Feature-cache manifests deduplicate summaries by logical cache identity before downstream consumption. The logical key is dataset, backbone, split section, and canonical split identity. Support caches use identities such as `shot_1_seed2`, `shot_2_seed2`, and `shot_16_seed2`; base caches use the canonical base split identity such as `base_seed2` plus the `train`/`val`/`test` split section. If an old metadata-incomplete summary and a newer explicit summary describe the same logical cache, the manifest keeps one preferred entry and records the old one under `ignored_stale_entries` in `feature_cache_manifest_summary.json`.

Stale feature summaries are ignored, not deleted. The manifest builder does not modify old `feature_extraction_summary.json` files, feature-cache tensors, metrics, or metadata. Consumer preflights must not add duplicate entries for the same support split unless future summaries introduce an explicit, documented shard contract with `shard_id`; the current project has no shard-merging semantics.
