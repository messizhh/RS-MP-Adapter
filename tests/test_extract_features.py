...
class ExtractFeaturesTest(unittest.TestCase):
    ...
    def test_guarded_real_extraction_writes_server_metadata_without_text_or_predictions(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIsNone(cache.metadata["open_clip_initial_pretrained"])
            self.assertTrue(cache.metadata["open_clip_initialization_warning_expected"])
            self.assertTrue(cache.metadata["checkpoint_load_happened_after_model_init"])
            self.assertTrue(cache.metadata["final_weights_loaded_from_checkpoint"])
            self.assertEqual(cache.metadata["final_weight_source"], "cli_override_checkpoint")
