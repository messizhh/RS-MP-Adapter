...
def write_fake_zero_shot_run(
    ...
    *,
    ...
    run_dir = root / "results" / "raw" / "eurosat" / "remoteclip_vit_b32" / "zero_shot" / "seed_1" / "fake_run"
    run_dir.mkdir(parents=True, exist_ok=True)
    (run_dir / "config.yaml").write_text("method:\n  name: zero_shot_clip\n", encoding="utf-8")
    (run_dir / "log.txt").write_text("Experiment log initialized.\nExperiment finished.\n", encoding="utf-8")
    metadata = base_metadata(run_dir, is_paper_result=is_paper_result, eligible_for_paper_tables=eligible_for_paper_tables)
...
def base_metadata(run_dir: Path, *, is_paper_result: bool, eligible_for_paper_tables: bool) -> dict[str, object]:
    return {
        "run_id": "fake_run",
        "method": "zero_shot",
        "dataset": "eurosat",
